...
def _get_last_page_offset(url_html):
    ...
    int
    """
    page_numbers = url_html.find_all("div", attrs={"class": "pageNumbers"})[0]
    last_page_offset = page_numbers.find_all("a")[-1].get("data-offset")
...
def get_restaurants_info(restaurants_list, url_html, thread_pool):
    ...
    def get_restaurant_info(restaurant_tag):
        ...
        restaurant_data = restaurant_html.find_all("script")[1].contents[0]
...
def _make_csv(restaurants_lists, city):
    columns = list(restaurants_lists[0].keys())
    df = DataFrame(restaurants_lists, columns=columns)
    df.to_csv( f'/Users/Nonis/desktop/Leads-OV-estimation/data/raw/ta_data6.csv', index=False)
...
if __name__ == "__main__":
    ...
    _make_csv(restaurants_data, args.city)